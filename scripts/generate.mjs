...
${faqBlock(p.faqs)}`;
  ...
  write(url, head(clampTitle(p.title), mkMeta(p.meta), url, schema, { pagetype: 'preise' }) + header + main + footer + sctaBar(waMsg) + calcJS + FOOT_JS + '</body></html>');
  written.push(url);
}
